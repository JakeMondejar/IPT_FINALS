...
int main() {
 while (FCGI_Accept() >= 0) {
    ...
    rc = sqlite3_open("/var/www/ipt/mondejar/cgi-bin/db/data.db", &db);
    ...
    const char *sql = (id == 0) ? "SELECT * FROM hotel WHERE id > ?" : "SELECT * FROM hotel WHERE id = ?";
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            json_object_set_new(object, "datein", json_string((const char*)sqlite3_column_text(stmt, 5)));
            json_object_set_new(object, "dateout", json_string((const char*)sqlite3_column_text(stmt,6)));
            json_object_set_new(object, "room", json_string((const char*)sqlite3_column_text(stmt, 7)));
            json_object_set_new(object, "guest", json_integer(sqlite3_column_int(stmt, 8)));
