...
void create_json_array(Report info) {
    ...
    json_object_set_new(obj, "datein", json_string(info.datein));
    json_object_set_new(obj, "dateout", json_string(info.dateout));
    json_object_set_new(obj, "room", json_string(info.room));
    json_object_set_new(obj, "guest", json_string(info.guest));
...
void loadJSON(Report *info) {  
    ...
    const char* properties[] = { "id", "name", "email", "phone", "address", "datein", "dateout", "room", "guest" };
    char** infovar[] = {&info->id,&info->name,&info->email,&info->phone,&info->address,&info->datein,&info->dateout,&info->room,&info->guest};
...
int main(void) {
  ...
  while (FCGI_Accept() >= 0) {
   ...
   const char *sql_statement = "insert into hotel values(?,?,?,?,?,?,?,?,?);";
   ...
   loadJSON(&info);
    ...
    dbres = sqlite3_open_v2("/var/www/ipt/mondejar/cgi-bin/db/data.db", &db, SQLITE_OPEN_READWRITE,NULL);
    ...
    dbres |= sqlite3_bind_text(stmt, 5, info.address, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 6, info.datein, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 7, info.dateout, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 8, info.room, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 9, info.guest, -1, SQLITE_TRANSIENT);
...
// curl -X POST -H "Content-Type: application/json" -d '[{"name": "id", "value": "2"},{"name": "name", "value": "Arnold"},{"name": "ticket", "value": "43256"}]' http://dummy.mshome.net:8081/lect/cgi-bin/get.o


//curl -X POST -H "Content-Type: application/json" -d '[{"name": "id", "value": "2"}' http://dummy.mshome.net:8081/lect/cgi-bin/get.o